...
class RLHFDataset(Dataset, ImageProcessMixin):
    ...
    def __init__(
        ...
        subset_ratio: float = 1.0, # 新增参数，默认1.0代表全用
    ):
        ...
        if data_split.lower() == "train" and subset_ratio < 1.0:
            total_len = len(self.dataset)
            keep_len = int(total_len * subset_ratio)
            self.dataset = self.dataset.select(range(keep_len))
