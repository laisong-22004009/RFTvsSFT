...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_model_only_checkpoint(self, path: Optional[str] = None):
        """
        Only load model parameters from checkpoint directory.
        Skip loading optimizer, scheduler, or extra states.
        """
        if path is None:
            return
        ...
        print(f"[rank-{self.rank}]: Loading model only from {os.path.abspath(model_path)}.")
        model_state_dict = torch.load(model_path, weights_only=False)

        state_dict_options = StateDictOptions(cpu_offload=True)

        # Pass empty optimizer tuple to satisfy API, but skip actual optimizer setting
        set_state_dict(
            model=self.model,
            optimizers=(),
            model_state_dict=model_state_dict,
