...
class RayPPOTrainer:
    ...
    def _get_ref_ckpt_path(self) -> str:
        """
        Get the reference checkpoint path.
        The reference checkpoint is used to load the model weights for the reference policy.
        """
        if self.config.trainer.ref_model_switch_steps is None:
            return None
        ref_ckpt_paths = self.config.trainer.ref_model_switch_paths
        print(f"Reference checkpoint paths: {ref_ckpt_paths}")
        # random随机返回一个
        ref_ckpt_path = ref_ckpt_paths[
            np.random.randint(0, len(ref_ckpt_paths))
        ] if isinstance(ref_ckpt_paths, list) else ref_ckpt_paths
        if not os.path.exists(ref_ckpt_path):
            raise ValueError(f"Reference checkpoint path {ref_ckpt_path} does not exist.")
        return ref_ckpt_path
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    # zhh ==== 动态切换 RefPolicy 权重（根据 global_step） ====
                    switch_step = getattr(self.config.trainer, "ref_model_switch_steps", None)
                    
                    should_switch_ref = (
                        self.use_reference_policy and
                        switch_step is not None and
                        self.global_step % switch_step == 0
                    )
                    
                    if should_switch_ref:
                        new_ckpt_path = self._get_ref_ckpt_path()
                        print(f"[Step {self.global_step}] Switching RefPolicy weights from: {new_ckpt_path}")
                        print(f"[DEBUG] ref_policy_wg type: {type(self.ref_policy_wg)}")
                        print(f"[DEBUG] load_ref_weights attr: {getattr(self.ref_policy_wg, 'load_ref_weights', 'not found')}")
                        self.ref_policy_wg.load_ref_weights(new_ckpt_path)
                        self.logger.log({"debug/ref_policy_switched": 1}, step=self.global_step)
