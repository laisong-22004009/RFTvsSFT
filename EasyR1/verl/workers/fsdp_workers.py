...
class FSDPWorker(Worker):
    ...
    def load_ref_weights(self, checkpoint_dir: str):
        """
        Online load new weights into RefPolicy from checkpoint dir (per-rank shards).
        """
        assert self._is_ref
        assert os.path.exists(checkpoint_dir), f"Checkpoint dir not found: {checkpoint_dir}"

        self.print_rank0(f"[RefPolicy] Loading weights from {checkpoint_dir}")
        checkpoint_mgr = FSDPCheckpointManager(
            model=self.fsdp_module,
            optimizer=None,
            lr_scheduler=None,
            processing_class=self.processor or self.tokenizer,
        )
        checkpoint_mgr.load_model_only_checkpoint(checkpoint_dir)
        self.print_rank0(f"[RefPolicy] Weights updated successfully from {checkpoint_dir}")
