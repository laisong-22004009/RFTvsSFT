            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    if "ref_log_probs" in model_inputs:
                        ...
                        # kl_error = kl_loss_value - target_kl
                        # adaptive_kl_coef = self.config.kl_coef * torch.exp(kl_error / target_kl)
                        # adaptive_kl_coef = adaptive_kl_coef.clamp(min=0.01, max=1)
                        # alpha = 0.8
                        # self.config.kl_coef = alpha * self.config.kl_coef + (1 - alpha) * adaptive_kl_coef.detach()
                        # pg_loss = pg_loss + kl_loss * self.config.kl_coef
                        # metrics["actor/kl_loss"] = kl_loss_value.item()
                        # metrics["actor/kl_coef"] = self.config.kl_coef.item()
